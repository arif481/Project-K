    ...
    return (
        <HudCard title="EVENT HORIZON">
            <div className="flex flex-col gap-4 relative">
                {/* Timeline Spine */}
                <div className="absolute left-[7px] top-2 bottom-2 w-[1px] bg-(--border-dim)" />
                ...
                {milestones.slice(0, maxItems).map((item, idx) => {
                    ...
                    return (
                        <div key={idx} className={`relative pl-6 flex flex-col ${isCompleted ? 'opacity-50 grayscale' : ''}`}>
                            {/* Dot Indicator */}
                            <div className={`absolute left-[3px] top-1.5 w-[9px] h-[9px] rounded-full border border-black z-10 ${isCurrent ? 'bg-(--neon-cyan) animate-pulse shadow-[0_0_10px_var(--neon-cyan)]' :
                                    isCompleted ? 'bg-(--text-dim)' : 'bg-(--bg-grid) border-(--border-bright)'
                                }`} />

                            <div className="flex justify-between items-start">
                                <span className={`text-xs font-mono font-bold ${isCurrent ? 'text-(--neon-cyan)' : 'text-white'}`}>
                                    {item.label}
                                </span>
